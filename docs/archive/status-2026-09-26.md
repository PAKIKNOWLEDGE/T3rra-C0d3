> **【归档 2026-09-26】** 2026-09-26 重构前的 `status.md` 全文（插入式日志，含多段已被推翻的记录）。现行状态见 [`docs/status.md`](../status.md)。
...
## 2026-09-26（批次 3）：ark 金标准迁移（第二刀）

**这次实际落地**：把现有控件按真实代码契约重新编排成 ARK 的工作区构图。左栏现在承载 `PROCESS / EVENTS`、真实会话列表和蓝色 `＋ NEW` 主操作；右栏只保留当前会话、静默信号、运行时选项、工具和专家传输事实。会话 tile 使用样张的编号、灰色渐变块和浅色选中块；消息、工具行和批准条进一步采用记录簿、站点卡和道具卡的材质语言。

**证据**：`npm run check`、`npm run check:app`、`npm run check:boundary` 均通过【实测】。本次没有改动引擎、ACP、传输或 `ConsoleView` 契约；所有控件 ID 与 `console.ts` 保持一致。

**已知未做 / 只做了一半**：真实引擎连接下的流式视觉和窄窗口布局仍需仓库主人目视；没有把样张中的假资源数、计划进度、diff 行数或工具计数硬搬进产品。目视对照样张后又把主舞台网格稀疏化、把 `＋ NEW SESSION` 提到左栏首位、移除档案栏角括号、修正审批条上下结构，并把工具卡底部的假进度条改成无数值含义的刻度线，减少底纹与 HUD 噪声对记录流的抢焦点。
