...
// Second dimension of the same lesson: the scan above only sees app/index.html, so every control
// the *renderer* creates (LOAD rows, the × delete button, ＋ CREATE SESSION, the RETRY button, mode
// chips, selects) was outside the manifest entirely — a control could be dead up there and the
// build stayed green. `dynamic` closes that hole: each element created in app/src/ui/** must be
// classified, matched by the id or className literal assigned near its creation site.
const UI_DIR = "app/src/ui";
const DYNAMIC_MANIFEST_KEY = "dynamic";

/** Identifying string literals (`x.id = "…"` / `x.className = "…"`) following a creation site. */
const createdElements = (source) => {
  const found = [];
  constCreation: for (const match of source.matchAll(/document\.createElement\("(button|select|input|textarea)"\)/g)) {
    const window = source.slice(match.index, match.index + 900);
    const labels = [...window.matchAll(/\.(?:id|className) = "([^"]+)"/g)].map((m) => m[1]);
    if (labels.length === 0) continue;
    found.push({ tags: match[1], labels: new Set(labels.flatMap((label) => label.split(/\s+/))) });
    void creation;
  }
  return found;
};
...
if (!existsSync(UI_DIR)) {
  ...
  for (const entry of created) {
    ...
    if (!covered) {
      failures.push(`${entry.file} creates a <${entry.tags}> (${[...entry.labels].join(" / ") || "no id or class"}) that ${MANIFEST} does not classify under "${DYNAMIC_MANIFEST_KEY}"`);
    }
  }
  // A declared-but-no-longer-created control is as stale as a shell entry pointing at nothing.
