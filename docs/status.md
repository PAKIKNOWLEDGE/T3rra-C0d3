...
- 这一任被解雇，原因写在 [`handover.md`](./handover.md) 开头：**把"已知未做"留在代码注释里、没放进汇报**。
  交接单已写好，接手的人从 [`handover.md`](./handover.md) 进入。
- **验收清单的计数是实话**：11 条里**已修 3 条**（#3 EVENTS 视图、#4 NEW、#11 闸门假阴性 + 元素清单制），
  **其余 8 条未做**（#1 中断、#2 会话列表+删除、#5 reasoning 收起、#6 输入框方框、#7 markdown 渲染、
  #8 锚点过大、#9 对话区重做、#10 审批界面）。**这张表就是验收标准，勾选权在主人手里。**
- 交接内容：可跑命令、真做完/半做/未做的三分法、下一步（中断与会话列表的端点与桥方案）、
  八条规矩（含上两任的翻车点）、证据地图。
...
| 11 | 闸门是假阴性（防的是谁） | **已修（本轮）** | 两步：① 嗅探"看着能点但不是控件"的元素；② **`app/ui-manifest.json` 清单制**——界面里每个能被看见/被点的元素都必须**显式分类**（`wired` 或 `static: 理由`），没分类、清单里有界面里没有、或声称 wired 但代码里没引用 → **构建失败**（已实测：删掉一条分类即红） |
