...
- **冷启动缺文档即停**：如果本次任务依赖的当前文档、权威引用或本文件指向的路径找不到、打不开或链接失效，立即停止实现，汇报准确路径和缺失项，交给仓库主人恢复或指定替代物。不得用记忆、旧归档、抓包推断或“看起来相近”的文件替代；只有主人明确处理后才能继续。
- **验收说明是交付必填项**：涉及界面或可见行为时，最终汇报必须同时写清：启动命令、可打开的 URL/文件路径、必要前置配置、3–5 个按顺序执行的用户动作、每一步的预期可见结果，以及独立列出的机读检查结果。无头禁令下，机器检查不能代替主人目视验收。

**界面**（视觉细则见 `docs/design.md`）

- **无头浏览器禁用**：不跑 puppeteer / playwright / jsdom，也不跑 ark-ui 的脚本。**不得声称看过渲染结果。** 交付界面时按上面的验收说明给出可打开路径、前置配置、3–5 个操作和预期结果，由仓库主人目视验收。
