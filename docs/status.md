...
| 2 | 会话列表 + 网页端删除 | **待做 · 最高优先** | 列表：ACP `session/list`；删除：HTTP `DELETE /session/{sessionID}`。**主人 2026-09-23 定为下一优先**：废弃会话在引擎侧越积越多，必须能删 |
...
**计数**：11 条中已修 **8**（#3–#9、#11），其余 **3 条待做**（#1 中断、#2 会话列表+删除、#10 审批）。**勾选权在仓库主人。**  
**优先级（2026-09-23，主人）**：**#2 会话列表 + 删除 = 最高优先**（废弃会话积压）→ 再 #10 审批 → #1 中断（可与 #2 共用 HTTP 通道）。  
...
| 优先级 | 类别 | 缺口 |
| --- | --- | --- |
| **P0** | 产品 | **验收 #2：会话列表 + 网页删除**（`session/list` + HTTP `DELETE`）；废弃会话在引擎侧积压，主人点名最高优先 |
| P1 | 产品 | 审批三档界面（#10）；断链/错误态；多会话 |
| P2 | 产品 | HALT（#1，可与 #2 共用 HTTP 透传） |
